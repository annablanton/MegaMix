class GrapplingHook {
    ...
    draw(ctx) {
        ctx.drawImage(this.rotatedHook, this.x - this.game.camera.x, this.y);
        this.rope.draw(ctx);
    }
}
...
class GrapplingRope {
    ...
    draw(ctx) {
        ctx.save();
        ctx.translate(this.ropeOriginX - this.game.camera.x, this.ropeOriginY);
        ctx.rotate(this.angle);
        ctx.translate(-this.ropeOriginX + this.game.camera.x, -this.ropeOriginY);

        ctx.drawImage(this.spritesheet, 16, 47, this.length / 2, 16, this.ropeOriginX - this.game.camera.x, this.ropeOriginY, this.length, 32)
        ctx.restore();
    }
}