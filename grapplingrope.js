class GrapplingHook {
    ...
    update() {
        ...
        if (!this.retracting) { //extend hook
            ...
            var dist = distance(this.megaman.x + this.megaman.FIRE_OFFSET_X + ellipsePoint.x, this.megaman.y + this.megaman.FIRE_OFFSET_Y + ellipsePoint.y, this.x, this.y) ;
    ...
    draw(ctx) {
        ctx.drawImage(this.rotatedHook, this.x, this.y);
        this.rope.draw(ctx);
    }
}
...
class GrapplingRope {
    ...
    draw(ctx) {
        ctx.save();
        ctx.translate(this.ropeOriginX, this.ropeOriginY);
        ctx.rotate(this.angle);
        ctx.translate(-this.ropeOriginX, -this.ropeOriginY);

        ctx.drawImage(this.spritesheet, 16, 47, this.length / 2, 16, this.ropeOriginX, this.ropeOriginY, this.length, 32)
        ctx.restore();
    }
}