...
class Powerup {
    constructor(game, x, y, type) {
        ...
        this.spritesheet = ASSET_MANAGER.getAsset("./sprites/powerup.png");
        this.emerging = true; //From a block when block being shot? 
        this.velocity = { x: 0, y: 0};
        this.updateBB();
        this.effectiveTime = 10000;
    ...
    update() {
        ...
        if (this.isEffective == true) {
            this.effectiveTime -= this.game.clockTick;
        }
        if (this.effectiveTime == 0) {
            this.shrinkPelletSize();
        }
    }
    
    draw(ctx) {

        ctx.drawImage(this.spritesheet, 0, 0, 336, 336, this.x - this.game.camera.x, this.y - this.game.camera.y, this.SPRITE_WIDTH, this.SPRITE_HEIGHT);
